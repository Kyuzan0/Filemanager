...
<body class="bg-slate-50 text-slate-900 dark:bg-[#0f1419] dark:text-slate-300 overflow-hidden">
    ...
    <div class="app h-screen flex overflow-hidden" id="app" role="application" aria-label="File Manager Application">
        ...
        <main class="main flex-1 h-full overflow-y-auto relative" id="main-content" role="main" aria-label="File Manager Main Content">
            <!-- HEADER ACTIONS -->
            <section class="header-actions mb-4 p-1.5 bg-white dark:bg-[#1a2332] border border-gray-200 dark:border-white/10 rounded-lg shadow-sm flex items-center justify-between gap-2 sticky top-0 z-20" role="toolbar" aria-label="File manager actions">
                <!-- Left Group: Navigation & Search -->
                <div class="flex items-center gap-2 flex-1 min-w-0">
                    <!-- Mobile Menu Toggle -->
                    <button class="btn btn-icon md:hidden p-0.5 hover:bg-slate-100 dark:hover:bg-white/5 rounded flex-shrink-0"
                    ...
                    <nav class="breadcrumbs hidden md:flex text-sm text-slate-600 dark:text-gray-400 min-w-0 flex-shrink"
                         id="breadcrumbs"
                         aria-label="Breadcrumb navigation">Home</nav>
                    
                    <!-- Search - Compact version for desktop -->
                    <div class="search hidden md:flex items-center gap-2 px-2 py-1.5 rounded-lg border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-white/5 flex-shrink-0 ml-auto" role="search">
                ...
                <div class="flex items-center gap-2 flex-shrink-0" role="group" aria-label="Create actions">
                    <button class="flex items-center gap-1.5 px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-md transition-colors shadow-sm"
                            id="newBtn"
                            aria-label="Create new file or folder (Ctrl+N)">
                ...
                <div class="flex items-center gap-2 flex-shrink-0" role="group" aria-label="Utility actions">
                    ...
                    <div class="items-center gap-2 px-2 py-1.5 bg-gray-50 dark:bg-white/5 rounded-md border border-gray-100 dark:border-white/5 hidden sm:flex" role="status" aria-live="polite">
                        <span class="text-xs font-medium text-gray-500 dark:text-gray-400" id="selectedCount" aria-label="Selection count">0 selected</span>
                        <div class="h-4 w-px bg-gray-300 dark:bg-white/10 mx-1" aria-hidden="true"></div>
                        <button class="text-gray-400 hover:text-red-500 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                                id="deleteSel"
                                title="Hapus"
            ...
            <nav class="footer mt-3 flex flex-col sm:flex-row items-center justify-between text-sm text-slate-600 dark:text-slate-400 gap-3 p-3 bg-white dark:bg-slate-900 border border-gray-200 dark:border-white/10 rounded-lg"
                ...
                <div id="showing" class="text-center sm:text-left" role="status" aria-live="polite">Menampilkan 0 dari 0 item</div>
                <div class="flex flex-wrap items-center justify-center gap-2">
                    ...
                    <div class="flex items-center gap-1" id="pagination-buttons" role="group" aria-label="Page navigation">
                        <button id="prevPage"
            ...
            <div class="loader-overlay fixed inset-0 hidden items-center justify-center bg-black/30 z-50 p-4"
                 ...
                 aria-modal="true"
                 aria-label="Loading">
                <div class="loader-inner px-4 py-3 rounded-md shadow flex items-center gap-3 max-w-xs w-full" role="status" aria-live="polite">
    ...
    <!-- MODAL UPLOAD -->
    <div class="modal-backdrop fixed inset-0 bg-black/45 hidden items-center justify-center z-50"
         id="modalBackdrop"
         role="dialog"
         aria-modal="true"
        ...
        <div class="modal" id="uploadModal">
            ...
            <div class="mb-4 flex items-center justify-center border-2 border-dashed border-slate-300 rounded-lg p-6 cursor-pointer hover:border-slate-400 transition-colors"
                 id="fileDropZone"
                 role="button"
                 tabindex="0"
                 aria-label="Click to select files or drag and drop files here">
                <div class="text-center">
                    <i class="ri-upload-cloud-line text-3xl text-slate-400 mb-2 block" id="uploadIcon" aria-hidden="true"></i>
            ...
            <input type="file" id="folderInput" webkitdirectory directory multiple class="hidden" aria-label="Select folder to upload">
            <div class="mb-4 text-sm text-slate-600 max-h-48 overflow-y-auto" id="fileList" role="list" aria-label="Selected files"></div>
            <div class="flex gap-2 justify-end" role="group" aria-label="Upload actions">
                ...
                <button class="btn btn-primary px-4 py-2 rounded-lg" id="doUpload" aria-label="Start upload">Unggah</button>
            </div>
        </div>
    </div>
